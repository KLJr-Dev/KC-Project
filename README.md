...
**Product tip:** SoftDev Notes + SSH landed on **`main`** (PR #21) — tag **`v1.2.0`** after examiner dry-run + sign-off ([pentest-ready](docs/release/v1.2.0-pentest-ready.md), unsigned until then).  
...
| 4 | tip on `main` → tag **`v1.2.0`** (pending tag) | tag **`v2.2.0`** (after Red) |
...
| `v1.2.0` | Cycle-4 SoftDev insecure tip (Notes XSS + SSH overlay) — **on `main`; tag pending** |
...
**ADR-015** defines the four SoftDev rails. **ADR-032** adds frozen `ctf/*` / `remediation/*` archives. Together that is **9 remotes today** (+ planned archives after tags).
...
| Frozen archives | `ctf/v1.1.0`, `ctf/leak-crack-db`, `remediation/v2.0.0`, `remediation/v2.1.0`, `remediation/cycle-3-leak-crack-db` | **Keep forever** (portfolio evidence) |
| Planned | `ctf/v1.2.0` (after tag), later `remediation/v2.2.0` | Create at those gates — not on origin yet |
...
main                              Product tip (SoftDev insecure Notes+SSH; pin v2.1.0 for secure demos)
 ...
 ├── ctf/v1.2.0                    Planned — archive from tag after v1.2.0 ship
