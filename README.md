...
## Current Status (tip **`v2.4.0`** — hardened · Cycle-7 Blue)

**Product tip:** Ops Documents path-confined; Cycle-7 overlays unpublished on default prod (ADR-035).  
**Cycle-7 insecure replay:** **`v1.4.0`** / **`ctf/v1.4.0`** + `docker-compose.cycle7.yml`.  
...
> **Note:** Hardened demos use tag **`v2.4.0`**. Do not use `v1.4.0` / CTF branches for recruiter “secure” walks.

Cycles **1–7 are closed**. Playable boxes on frozen `ctf/*` (see [security README](docs/security/README.md)).
...
| 7 | tag **`v1.4.0`** / `ctf/v1.4.0` | tag **`v2.4.0`** · frozen `remediation/v2.4.0` |

| Track | Artifact |
|-------|----------|
| Blue / secure tip | **`v2.4.0`** — [v2.4.0-secure-ready.md](docs/release/v2.4.0-secure-ready.md) |
| Cycle-7 Red (frozen) | `ctf/v1.4.0` — [writeup](https://github.com/KLJr-Dev/KC-Project/blob/ctf/v1.4.0/docs/security/Cycle-7/PenTest/v1.4.0-writeup.md) |
...
- **v1.4.0 Cycle-7 replay:** pin tag/`ctf/v1.4.0` + `docker-compose.cycle7.yml` — not default for secure tip
...
- **Product UI:** My Files, Notes, Link Preview, **Ops Documents**, Sharing, Review (mod), Admin — `/dev` gated unless lab flag
- **Tests:** smoke (incl. cycle6 + cycle7 blue asserts) · journey · e2e-docker · tls-smoke
- **Status:** Cycle-7 **closed** at **`v2.4.0`** — [Cycle-7](docs/security/Cycle-7/README.md)
...
| Kind | Remotes | Policy |
|------|---------|--------|
| Product tip | `main` | Secure tip (`v2.4.0`); pin tags for demos |
...
```
main                              Product tip (v2.4.0 secure; pin v1.4.0 for Cycle-7 replay)
 ├── backend                       Feature lane — Nest/API
 ├── frontend                      Feature lane — Next UI
 ├── dev                           Feature lane — integration → PR main
 ├── ctf/v1.1.0 … ctf/v1.4.0       Frozen Red / CTF archives
 ├── ctf/leak-crack-db             Frozen Cycle-3
 ├── ctf/shells-privesc            Frozen Cycle-5
 └── remediation/v2.0.0 … v2.4.0  Frozen Blue history (+ cycle-3 / shells-privesc)
