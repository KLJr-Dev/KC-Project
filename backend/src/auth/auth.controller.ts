/**
 * M6 / v2.0.0 — AuthController (register / login / refresh / logout / me).
 *
 * Security measures:
 * - CWE-922: Refresh token set as httpOnly cookie (cookie.util); never left
 *   in the JSON body returned to the browser.
 * - CWE-352: POST /auth/refresh requires X-Requested-With (csrf.util) in
 *   addition to SameSite cookie attributes.
