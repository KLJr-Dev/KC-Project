...
export default function DevSharingDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setResult(JSON.stringify(res, null, 2));
      setTimeout(() => router.push('/dev/sharing'), 600);
    } catch (err) {
      setError(String(err));
    }
  };
  ...
  return (
    <div className="space-y-8">
      <Link href="/dev/sharing" className="text-sm text-muted underline hover:text-foreground">
        ← Back to Sharing explorer
      </Link>
      <h1 className="text-xl font-semibold text-foreground">Sharing {id}</h1>
      ...
      <form onSubmit={handleUpdate} className="space-y-3">
        <h2 className="text-sm font-medium text-muted">PUT /sharing/{id}</h2>
      ...
      <div className="space-y-2">
        <h2 className="text-sm font-medium text-muted">DELETE /sharing/{id}</h2>
        <button
