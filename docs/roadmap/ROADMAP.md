...
Canonical pairs and CTF-only policy: [STRATEGY.md](STRATEGY.md) · [ADR-032](../decisions/ADR-032-post-v2.1.0-versioning.md) · [ADR-033](../decisions/ADR-033-cycle-4-softdev-version-pair.md) · [ADR-034](../decisions/ADR-034-cycle-6-product-expansion-pair.md).

- **v1.0.0 / v2.0.0** — Cycle-1 pair (closed)
- **v1.1.0 / v2.1.0** — Cycle-2 pair (closed)
- **Cycle-3** — CTF-only (`ctf/leak-crack-db`); no product bump
- **v1.2.0 / v2.2.0** — Cycle-4 **closed**: Notes XSS + SSH → harden Notes / no default SSH
- **Cycle-5** — CTF-only (`ctf/shells-privesc`); no product bump
- **v1.3.0 / v2.3.0** — Cycle-6 product expansion **in design**: Link Preview SSRF + CSRF ([Cycle-6](../security/Cycle-6/README.md))
