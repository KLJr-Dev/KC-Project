...
## Frontend Surface (v0.3.x)

- Files page (`frontend/app/files/page.tsx`) uses real `<input type="file">` and calls the multipart `/files` endpoint, lists files via `GET /files`, and exposes direct download links to `GET /files/:id/download`.
- Sharing page (`frontend/app/sharing/page.tsx`) creates public shares, surfaces `publicToken` values, and exposes unauthenticated download URLs for `GET /sharing/public/:token`.
